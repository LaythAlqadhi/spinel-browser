  ...
  persist(
    (set, get) => ({
      ...
      initializeStore: async () => {
        ...
        // Only create a tab if there are no tabs at all after loading from persistence
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          // Convert date strings back to Date objects for tabs
          if (state.tabs) {
          ...
          if (state.bookmarkFolders) {
            state.bookmarkFolders = state.bookmarkFolders.map((folder: any) => ({
              ...folder,
          ...
          if (state.history) {
            ...
            if (state.settings?.clearHistoryOnExit) {
              ...
              state.history = state.history.map((entry: any) => ({
                ...entry,
...
// Convenience selectors for easier access to specific parts of the state
